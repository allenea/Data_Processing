        ...
        for dtype1 in dtype_subusage:
            ...
            for datype in dtype_usage:
                ...
                if isRaw == True:
                    ...
                    outdir =  mydir+"/plots/All_Data/AVG/LINE/"
            ...
            for fileDIR in filelst:
                for file in glob.glob(fileDIR+'*.csv'):    
                    ...
                    if ID_String[0] == "CMLF":
                        #waterWinds.append(variable_data.tolist())  ## KEEP COMMENTED OUT
                        if isCMLF == False:
                            isCMLF = True
                            ax.plot(time,variable_data,'green',linewidth=4,label="CMLF")
                        else:
                            ax.plot(time,variable_data,'green',label="CMLF")
                        ...
                        if isDEOS2V == False:
                            ax.plot(time,variable_data,'magenta',label="DEOS Verification")
                            isDEOS2V = True
                        else:
                            ax.plot(time,variable_data,'magenta')
                        
                    elif ID_String[0][0] == 'K' and len(ID_String[0]) == 4: 
                        landWinds.append(variable_data.tolist())        
                        if isASOS == False:
                            ax.plot(time,variable_data,'blue',label="Local Climate Data")
                            isASOS = True
                        else:
                            ax.plot(time,variable_data,'blue')
                        
                    elif ID_String[0] == 'DELD1':
                        waterWinds.append(variable_data.tolist())      
                        ax.plot(time,variable_data,'purple')
                        
                    elif "DEL" in ID_String[0]:
                        landWinds.append(variable_data.tolist())
                        if isDelDOT == False:
                            ax.plot(time,variable_data,'teal',label="DelDOT")
                            isDelDOT = True
                        else:
                            ax.plot(time,variable_data,'teal')
                        
                    elif ID_String[0][0] == 'D' and len(ID_String[0]) == 4:
                        landWinds.append(variable_data.tolist())
                        if isDEOS1DA == False:
                            isDEOS1DA = True
                            ax.plot(time,variable_data,'tomato',label="DEOS Assimilation")
                        else:
                            ax.plot(time,variable_data,'tomato')
                            
                    elif len(ID_String[0]) == 4 and ID_String[0][0] == 'M' or ID_String[0] == 'SACM' or ID_String[0] == 'SBLV':
                        landWinds.append(variable_data.tolist())
                        if isNJMET == False:
                            isNJMET = True
                            ax.plot(time,variable_data,'brown',label="NJMET Verification")
                        else:
                            ax.plot(time,variable_data,'brown')
                        
                    else:
                        waterWinds.append(variable_data.tolist())
                        if isNDBC == False:
                            isNDBC = True
                            ax.plot(time,variable_data,'purple',label="NDBC")
                        else:
                            ax.plot(time,variable_data,'purple')
            ...
            if isRaw == True:
                ...
                if "Wind_Speed (m/s)" in variable and dtype1 == "10m":
                    ...
                    plt.title("30 Minute Averaged Wind Speeds Observed On "+case_time, fontsize=20)
    
                else:
                    plt.title("30 Minute Averaged "+variable+" Observed On "+case_time, fontsize=20)
    
            if isRaw == True:
                plt.savefig(outdir+"Time-Series_obs_"+dtype1+"_"+variable[:-6]+"_"+case_time[0:10]+"LINE.png")
            else:
                plt.savefig(outdir+"Time-Series_avg_"+dtype1+"_"+variable[:-6]+"_"+case_time[0:10]+"LINE.png")
